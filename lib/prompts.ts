...
export const garmentPrompt = ({ userPrompt, hasReference, view = 'front' }: GarmentParams): string => {
  ...
  return [
    ...
    ``,
    `STRICT GARMENT RULES (must follow exactly):`,
    `• Produce EXACTLY ONE single garment in the image — never an outfit, never a top and bottom together, never layered pieces, never an accessory.`,
    `• The garment MUST be one of these offered types only: Crew Neck Sweatshirt, Hoodie, Long Sleeve T-Shirt, T-Shirt, Sweat Pants, Open-Bottom Sweatpants, Track Jacket, or Zip-Up Hoodie.`,
    `• If the user's request is not one of these (e.g. jeans, dress, shorts, jacket), interpret it as the SINGLE closest offered type and render only that.`,
    `• Do not invent garment categories outside this list. One garment per image, nothing else worn or shown.`,
    ``,
    `Design priorities:`,

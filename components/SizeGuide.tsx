...
export default function SizeGuide({
  ...
  return (
    <div className="p-4 md:p-6 max-w-[900px] mx-auto">
      ...
      {/* Garment selector */}
      {allowGarmentSwitch && (
        <div className="mb-8 flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <div className="relative group/tip">
              ...
              <div className="absolute top-full left-0 mt-2 w-60 bg-grace-ink text-white text-[11px] leading-relaxed rounded-xl px-3 py-2.5 opacity-0 group-hover/tip:opacity-100 pointer-events-none transition-opacity z-20">
                Generated by GRACE from your selected fit. Measurements shown flat in inches — half-measurements (Chest, Waist, Thigh) double for full circumference. Graded XS–3XL.
              </div>
            </div>
            {guide.edited && !editing && (
              <span className="text-[11px] font-semibold text-grace-red">Edited</span>
            )}
          </div>
        </div>
        {editable && !editing && (
      ...
      <div className="mt-4 flex items-start justify-between gap-4">
        <span className="text-[10px] text-grace-stone">{fitLabel(resolvedFit)} fit block · XS–3XL</span>
        {editing && (
          <button
            onClick={resetFit}
